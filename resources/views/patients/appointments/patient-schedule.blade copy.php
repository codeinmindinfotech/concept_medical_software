            ...
            <div class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <div class="input-group">
                  <input id="modal-appointment-date" name="appointment_date" type="text" class="form-control flatpickr @error('dob') is-invalid @enderror"
...
<script>
    ...
    async function loadSlotsAndAppointments() {
        ...
        // Check if the clinic is open that day
        if (!clinic[dayName]) {
            document.getElementById('slot-body').innerHTML = `<tr><td colspan="6">Clinic is closed on this day.</td></tr>`;
            return;
        }

        const interval = parseInt(clinic[`${dayName}_interval`] || 15);
        const amStart = clinic[`${dayName}_start_am`];
        const amEnd = clinic[`${dayName}_finish_am`];
        const pmStart = clinic[`${dayName}_start_pm`];
        const pmEnd = clinic[`${dayName}_finish_pm`];
        ...
        if (amStart && amEnd) {
            slots = slots.concat(generateTimeSlots(amStart, amEnd, interval));
        }

        if (pmStart && pmEnd) {
            slots = slots.concat(generateTimeSlots(pmStart, pmEnd, interval));
        }
        ...
        window.currentSlotDuration = interval;
    ...
    document.getElementById('bookAppointmentForm').addEventListener('submit', async function (e) {
        ...
        const data = {
            ...
            clinic_id: selectedClinic,
            apt_slots: parseInt(form.querySelector('input[name="apt_slots"]:checked').value)
        };
    ...
    // document.getElementById('bookAppointmentModal').addEventListener('shown.bs.modal', function () {
    //     $('#appointment_type').select2({
    //         dropdownParent: $('#bookAppointmentModal')
    //     });
    // });
