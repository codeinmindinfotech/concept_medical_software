    ...
    <form action="{{ route('fee-notes.update', ['patient' => $patient->id, 'fee_note' => $feeNote]) }}" class="validate-form" method="POST">
        ...
        <div class="row g-3">
            ...
            <div class="mb-3 col-md-4">
                ...
                <select name="chargecode_id" id="chargecode_id" class="select2" required>
                    ...
                    @foreach($chargecodes as $code)
                        <option value="{{ $code->id }}" {{ old('chargecode_id', $feeNote->chargecode_id) == $code->id ? 'selected' : '' }}>
                            {{ $code->code }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <textarea name="description" class="form-control">{{ old('description', $feeNote->description) }}</textarea>
            </div>
            ...
            <div class="col-md-2">
                <label>Qty</label>
                <input type="number" name="qty" class="form-control" value="{{ old('qty', $feeNote->qty ?? 1) }}">
            </div>

            {{-- Gross --}}
            <div class="col-md-2">
                <label>Gross</label>
                <input type="number" name="charge_gross" class="form-control" value="{{ old('charge_gross', $feeNote->charge_gross) }}">
            </div>

            {{-- Reduction Percent --}}
            <div class="col-md-2">
                <label>Reduction %</label>
                <input type="number" name="reduction_percent" class="form-control" value="{{ old('reduction_percent', $feeNote->reduction_percent) }}">
            </div>

            {{-- Net --}}
            <div class="col-md-2">
                <label>Net</label>
                <input type="number" name="charge_net" class="form-control" value="{{ old('charge_net', $feeNote->charge_net) }}">
            </div>

            {{-- VAT Percent --}}
            <div class="col-md-2">
                <label>VAT %</label>
                <input type="number" name="vat_rate_percent" class="form-control" value="{{ old('vat_rate_percent', $feeNote->vat_rate_percent) }}">
            </div>

            {{-- Total --}}
            <div class="col-md-2">
                <label>Total</label>
                <input type="number" name="line_total" class="form-control" value="{{ old('line_total', $feeNote->line_total) }}" readonly>
            </div>
        </div>
            ...
            <div class="col-md-3">
                ...
                <select name="clinic_id" class="form-select select2">
                    ...
                    @foreach($clinics as $clinic)
                        <option value="{{ $clinic->id }}" {{ old('clinic_id', $feeNote->clinic_id) == $clinic->id ? 'selected' : '' }}>
                            {{ $clinic->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3">
                ...
                <select name="consultant_id" class="select2">
                    ...
                    @foreach($consultants as $consultant)
                        <option value="{{ $consultant->id }}" {{ old('consultant_id', $feeNote->consultant_id) == $consultant->id ? 'selected' : '' }}>
                            {{ $consultant->name }}
                        </option>
                    @endforeach
                </select>
